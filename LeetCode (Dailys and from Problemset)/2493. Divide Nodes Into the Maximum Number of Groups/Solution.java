...
class Solution {
  ...
  private void dfsComponents(int component, int node, Map<Integer, List<Integer>> graph,
      Map<Integer, List<Integer>> components, Set<Integer> visited) {
    components.get(component).add(node);
    for (int neighbor : graph.get(node)) {
      // happens when there is more than one spanning tree. I.E you can arrive at a
      // node in more than one way
      ...
      for (int i = 0; i < n; i++) {
        ...
        for (int neighbor : graph.get(curNode)) {
          if (curLevel.contains(neighbor))
            return -1; // Colored with different color == not bipartite
          if (visited.contains(neighbor))
            continue;
