...
// boolean valid = true;
// if (row == 0 || row == m - 1) { // one part is single row, we can only remove
// from two end
// if (position[1] != 0 && position[1] != n - 1)
// valid = false;
// } else if (col == 0 || col == n - 1) { // one part is single column, we can
// only remove from two end
// if (position[0] != 0 && position[0] != m - 1)
// valid = false;
// }
// if (row > 0 && n == 1) { // for single column matrix, only three cell are
// safe to remove
// if (position[0] != 0 && position[0] != m - 1 && position[0] != row)
// valid = false;
// } else if (col > 0 && m == 1) { // for single row matrix, only three cell are
// safe to remove
