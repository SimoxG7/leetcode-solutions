...
class Solution {
  public long minimumCost(String source, String target, String[] original, String[] changed, int[] cost) {
    ...
    long UPPER_BOUND = 1_000_000_000_000_000L;
    long[][] dist = new long[idCounter][idCounter];
    for (int i = 0; i < idCounter; i++) {
      Arrays.fill(dist[i], UPPER_BOUND);
      dist[i][i] = 0;
    }
    ...
    for (int k = 0; k < idCounter; k++) {
      for (int i = 0; i < idCounter; i++) {
        for (int j = 0; j < idCounter; j++) {
          if (dist[i][k] < UPPER_BOUND && dist[k][j] < UPPER_BOUND) {
    ...
    return dp[n] >= UPPER_BOUND ? -1 : dp[n];
  }
}